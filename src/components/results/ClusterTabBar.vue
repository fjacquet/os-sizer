...
<template>
  <div
    class="flex flex-wrap items-center gap-1 mb-4 border-b border-gray-200 dark:border-gray-700 pb-0"
  >
    <!-- Cluster tabs -->
    <div v-for="(cluster, idx) in clusters" :key="cluster.id" class="relative">
      <button
      ...
      >
        ...
        >
          {{
            roleOption === 'hub'
              ? t('results.clusters.roleHub')
              : roleOption === 'spoke'
                ? t('results.clusters.roleSpoke')
    ...
    <button
      ...
      :title="
        clusters.length >= MAX_CLUSTERS ? t('results.clusters.maxClustersReached') : undefined
      "
  ...
  <div v-if="openDropdownId !== null" class="fixed inset-0 z-9" @click="closeDropdown()" />
</template>
