  ...
  <div class="overflow-x-auto -mx-3 px-3 sm:mx-0 sm:px-0">
    <table class="w-full text-sm border-collapse" :aria-label="t('results.title')">
      <thead class="bg-gray-50 dark:bg-gray-700">
        <tr>
          <th
            class="px-3 py-2 text-left font-medium text-gray-600 dark:text-gray-300 border-b dark:border-gray-600"
          >
            {{ t('topology.label') }}
          </th>
          <th
      ...
      <tbody>
        <tr v-for="row in rows" :key="row.labelKey">
          <td
            class="px-3 py-2 border-b border-gray-100 dark:border-gray-700 text-gray-800 dark:text-gray-200"
          >
            {{ t(row.labelKey) }}
          </td>
          <td
        ...
        <tr v-if="props.result.sizing.rhoaiOverhead" class="bg-blue-50 dark:bg-blue-900/20">
          ...
          >
            ...
            <span class="text-xs text-gray-500 dark:text-gray-400 block not-italic">
              {{ t('rhoai.kserve') }} · {{ t('rhoai.dsPipelines') }} ·
              {{ t('rhoai.modelRegistry') }}
            </span>
          </td>
          <td
            class="px-3 py-2 border-b border-gray-100 dark:border-gray-700 text-gray-500 dark:text-gray-400 text-xs italic"
            colspan="4"
          >
            +{{ props.result.sizing.rhoaiOverhead.vcpu }} vCPU / +{{
