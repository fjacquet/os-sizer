...
<template>
  ...
  >
    ...
    <VirtWorkloadSection v-if="mode === 'virtualization'" />
    <template v-else>
      <!-- Workload inputs -->
        ...
        <div class="space-y-2">
          ...
          <div v-if="virtEnabled" class="ml-6 mt-2">
            <label class="flex items-center gap-2 cursor-pointer">
              ...
              />
              <span class="text-sm text-gray-700 dark:text-gray-300">{{
                t('workload.rwxStorageAvailable')
              }}</span>
            </label>
          </div>
          ...
          <div v-if="gpuEnabled" class="ml-6 mt-2 space-y-3">
            ...
            <div class="space-y-1">
              ...
              >
                ...
                <option v-for="profile in availableMigProfiles" :key="profile" :value="profile">
                  {{ profile }}
                </option>
              </select>
            </div>
            ...
            <div v-if="gpuMode === 'vgpu'" class="mt-2 space-y-1">
              <p class="text-xs font-semibold text-gray-700 dark:text-gray-300">
                {{ t('gpu.densityTableTitle') }}
              </p>
              <div class="overflow-x-auto">
                <table
                  class="w-full text-xs border-collapse border border-gray-200 dark:border-gray-600"
                >
                  <thead class="bg-gray-50 dark:bg-gray-700">
                    <tr>
                      <th class="px-2 py-1 border border-gray-200 dark:border-gray-600 text-left">
                        GPU
                      </th>
                      <th class="px-2 py-1 border border-gray-200 dark:border-gray-600 text-left">
                        Profile
                      </th>
                      <th class="px-2 py-1 border border-gray-200 dark:border-gray-600 text-right">
                        Instances
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr
                      v-for="(instances, profile) in MIG_PROFILES['A100-40GB']"
                      :key="`a100-40-${profile}`"
                    >
                      <td
                        class="px-2 py-1 border border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-400"
                      >
                        A100-40GB
                      </td>
                      <td class="px-2 py-1 border border-gray-200 dark:border-gray-600 font-mono">
                        {{ profile }}
                      </td>
                      <td
                        class="px-2 py-1 border border-gray-200 dark:border-gray-600 font-mono text-right"
                      >
                        {{ instances }}
                      </td>
                    </tr>
                    <tr
                      v-for="(instances, profile) in MIG_PROFILES['A100-80GB']"
                      :key="`a100-80-${profile}`"
                    >
                      <td
                        class="px-2 py-1 border border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-400"
                    ...
                    <tr
                      v-for="(instances, profile) in MIG_PROFILES['H100-80GB']"
                      :key="`h100-${profile}`"
                    >
                      <td
                        class="px-2 py-1 border border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-400"
