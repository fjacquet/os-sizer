# Requirements Archive: v1.0 Full OpenShift Sizer
...
- [x] **FORM-01**: Multi-step wizard: (1) Environment constraints → (2) Workload profile → (3) Architecture selection → (4) Results
- [x] **FORM-02**: Environment constraint inputs: datacenter / edge / far-edge / cloud, connectivity (connected / air-gapped), HA level required
- [x] **FORM-03**: Workload profile inputs: number of applications, total pods, average CPU per pod (millicores), average RAM per pod (MiB)
- [x] **FORM-04**: Architecture selection: auto-recommended or manual override from all 8 topologies
- [x] **FORM-05**: Optional add-ons toggle: ODF storage, infra nodes, GPU nodes, RHACM hub <!-- gap-closure Phase 6 -->
- [x] **FORM-06**: SNO profile selector: Standard / Edge / Telecom-vDU (different hardware minimums)
- [x] **FORM-07**: HCP inputs: number of hosted clusters, target QPS per cluster
- [x] **FORM-08**: All inputs validated with Zod; errors displayed inline
- [x] **FORM-09**: NumberSliderInput and WarningBanner shared components reused from vcf-sizer pattern

### Results Display

- [x] **RES-01**: On-screen Bill of Materials table: node type, count, vCPU, RAM, storage per node type <!-- gap-closure Phase 6 -->
- [x] **RES-02**: Total resource summary: total vCPU, total RAM, total storage
- [x] **RES-03**: Architecture overview card with topology name, HA level, and key constraints
...
- [x] **RES-06**: Warnings shown when inputs fall below official Red Hat minimums
- [x] **RES-07**: Charts (bar/donut) showing resource distribution across node types

### URL Sharing

- [x] **SHARE-01**: All wizard inputs encoded into a shareable compressed URL (lz-string + Zod schema)
- [x] **SHARE-02**: URL decoded on page load to restore full state
- [x] **SHARE-03**: Copy-to-clipboard button on results page

### Exports

- [x] **EXP-01**: PowerPoint (PPTX) export via pptxgenjs — architecture summary slide + BoM table slide
- [x] **EXP-02**: PDF export via jsPDF + jspdf-autotable — same content as PPTX, dynamically imported
- [x] **EXP-03**: CSV export — BoM table as comma-separated file, plain TypeScript blob download
...
- [x] **QA-01**: Unit tests for all engine sizing formulas (vitest)
- [x] **QA-02**: Unit tests for recommendation engine
- [x] **QA-03**: Unit tests for Zod validation schemas
